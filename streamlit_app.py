...
with tab3:
    ...
    <div class="doc-section">
        <h3 style='color:#ffd700; margin-top:0;'>📖 SENTINEL V26 算法引擎实战研判与仓位风控手册</h3>
        <p style='color:#b4c6d8; font-size:0.9rem;'>本手册旨在解构双引擎模型的底层数理逻辑，指导交易员如何结合宏观风控墙、微观个股指标进行科学的资金分配与止管执行。</p>
        
        <hr style='border-color:#333;'>
        
        <h4 style='color:#ffaa00;'>一、 🇨🇳 如何研判“大盘宏观环境风控墙”</h4>
        <p>系统顶部的风控墙通过实时跟踪<b>沪深300（核心资产）</b>、<b>上证指数（大盘权重）</b>、<b>创业板指（科技成长）</b>和<b>中证500（中盘标杆）</b>的 20日生命线（MA20），计算多头共振权重：</p>
        <ul style='font-size:0.85rem; color:#e0e6ed; line-height:1.6;'>
            <li><b style='color:#ff4d4d;'>四盘多头共振 (乘数 x1.30)：</b>全盘站在 MA20 之上。市场处于强动能周期，赚钱效应极佳，模型会全面释放个股评分，提示积极主攻。</li>
            <li><b style='color:#ffaa00;'>指数结构分化 (乘数 x1.00)：</b>部分指数破位，部分站稳。市场进入存量博弈或板块轮动慢牛。此时评分回归基准，应严格执行个股精选，避开破位板块。</li>
            <li><b style='color:#00ffaa;'>系统性多头退潮 (乘数 x0.70)：</b>大面积跌破 MA20。市场面临系统性回撤风险。模型会大幅压低所有资产的综合核心评分，此时<b>应主动降低总仓位，严防左侧抄底</b>。</li>
        </ul>

        <hr style='border-color:#333;'>

        <h4 style='color:#ffaa00;'>二、 📊 报告核心指标深度解构与战术用法</h4>
        <p>量化报告中的每一个维度都有明确的数理统计支撑，具体用法如下：</p>
        
        <table style='width:100%; border-collapse:collapse; font-size:0.85rem; color:#e0e6ed; margin-bottom:15px;'>
            <thead>
                <tr style='background:#1f242e; text-align:left;'>
                    <th style='padding:10px; border:1px solid #333; color:#ffd700;'>核心指标</th>
                    <th style='padding:10px; border:1px solid #333; color:#ffd700;'>数理逻辑与作用</th>
                    <th style='padding:10px; border:1px solid #333; color:#ffd700;'>实战具体用法</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td style='padding:10px; border:1px solid #333;'><b>数学期望值 (EV)</b></td>
                    <td style='padding:10px; border:1px solid #333;'>利用 14日 ATR 动态推演盈亏比空间，计算公式为：<br>$$ \\text{EV} = (\\text{修正胜率} \\times \\text{预期盈幅}) - (\\text{败率} \\times \\text{预期亏幅}) $$<br>代表单次博弈的理论平均净收益率。</td>
                    <td style='padding:10px; border:1px solid #333;'><b style='color:#ff4d4d;'>绝对核心。</b> 必须为正数方可参与博弈。EV 越高，代表当前形态下的风险收益比对多头越有利。</td>
                </tr>
                <tr>
                    <td style='padding:10px; border:1px solid #333;'><b>动态修正胜率</b></td>
                    <td style='padding:10px; border:1px solid #333;'>随机森林基于量比、乖离率、RSI 计算出历史基准胜率后，根据<b>日内实时K线（如冲高回落幅度、尾盘跳水异动）</b>进行动态高频扣减后的真实胜率。</td>
                    <td style='padding:10px; border:1px solid #333;'>若修正胜率显著低于基准胜率，说明盘中多头力量在衰竭（长上影诱多），即使历史形态再好，也应放弃当天追入。</td>
                </tr>
                <tr>
                    <td style='padding:10px; border:1px solid #333;'><b>预期周期</b></td>
                    <td style='padding:10px; border:1px solid #333;'>系统自动识别资产属性：<br>个股标准周期为 5-10 交易日；<br>普通 ETF 压缩至指数趋势跟踪；<br>杠杆资产则严格锁定在 2-5 交易日的超短线。</td>
                    <td style='padding:10px; border:1px solid #333;'>用于指导时间成本控制。如果持股达到预期周期上限仍未触及止盈或止损，通常意味着动能衰竭，应考虑时间止损换筹。</td>
                </tr>
                <tr>
                    <td style='padding:10px; border:1px solid #333;'><b>建议买入价</b></td>
                    <td style='padding:10px; border:1px solid #333;'>基于当前价格下摆 0.8% 作为基准挂单点。</td>
                    <td style='padding:10px; border:1px solid #333;'>拒绝市价单左侧追高。推荐使用<b>下摆网格或分批限价单</b>在建议价附近埋伏，等待日内波动提供流动性击穿时被动吸筹。</td>
                </tr>
                <tr>
                    <td style='padding:10px; border:1px solid #333;'><b>推荐止盈 / 止损点</b></td>
                    <td style='padding:10px; border:1px solid #333;'>摒弃固定百分比，完全基于 14日平均真实波幅 (ATR) 自动测算。针对高波动杠杆资产自动放大阈值（避免被震荡洗出），针对低波动普通 ETF 自动收窄。</td>
                    <td style='padding:10px; border:1px solid #333;'>盘前必须将此处输出的止盈止损点录入交易系统。一旦价格无条件触及支撑或阻力，必须斩断主观幻想，坚决执行交易纪律。</td>
                </tr>
                <tr>
                    <td style='padding:10px; border:1px solid #333;'><b>综合核心评分</b></td>
                    <td style='padding:10px; border:1px solid #333;'>计算公式：$$ \\text{Score} = \\text{动态修正胜率} \\times \\text{EV} \\times \\text{大盘风险权重} \\times 1000 $$</td>
                    <td style='padding:10px; border:1px solid #333;'>全量扫描或多股诊断时的<b>最终资金优选依据</b>。分值越高，说明大盘、个股动能、数理期望共振最强。</td>
                </tr>
            </tbody>
        </table>
        ...
        <h4 style='color:#ffaa00;'>三、 🛡️ 凯利公式（Kelly Criterion）导向的仓位管理方案</h4>
        <p>在实际交易中，哪怕个股评分再高，也严禁满仓孤注一掷。系统推荐采用量化领域经典的<b>凯利公式变体</b>来控制单资产的最高资金占用：</p>
        
        <div style='background:#0e1117; padding:15px; border-radius:8px; border:1px solid #444; margin-bottom:15px;'>
            <p style='margin:0; font-size:1rem; color:#ffd700; text-align:center;'>
                $$ \\text{建议单股最高仓位 } (f^*) = \\frac{\\text{动态修正胜率} \\times (b + 1) - 1}{b} \\times \\text{大盘风险权重} $$
            </p>
            <p style='margin:10px 0 0 0; font-size:0.8rem; color:#a3b3c2; text-align:center;'>
                * 其中 $b$ 为当前自适应盈亏比（$ \\text{预期盈幅} \\div \\text{预期亏幅} $）
            </p>
        </div>

        <ul style='font-size:0.85rem; color:#e0e6ed; line-height:1.6;'>
            <li><b>分批建仓策略：</b> 以【建议买入价】为核心。在建议价、建议价 $ - 0.5\\% $、建议价 $ - 1.2\\% $ 采用 <b>3:3:4</b> 的比例呈金字塔状布置限价单。</li>
            <li><b>实时风险提示拦截：</b> 
                <br>🚨 若输出触发 <b style='color:#ff4d4d;'>“机构无底线杀跌 / 盘中多头崩溃”</b>，证明微观结构遭到严重破坏，即使凯利公式计算有仓位，也必须放弃当天入场，严禁左侧接飞刀。
                <br>⚡ 若触发 <b style='color:#ffaa00;'>“杠杆工具特殊提示”</b>，由于此类资产带有内耗属性和波动放大效应，建议将凯利公式计算出的仓位再<b>主动减半</b>执行，以应对损耗。
            </li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
